...
- **Super-smysly ve fázi Observe**: Bot si automaticky načítá status trhu (Market Clock), makroekonomický kalendář, nadcházející korporátní události (dividendy, splity) a skenuje Alpaca Screener pro detekci "Top Movers" (největších skokanů a propadáků dne) ještě předtím, než začne přemýšlet.
- **Plná Autonomie a Risk Management**: Systém nepoužívá pevně dané úrovně pozic. LLM model si sám určuje velikost investice do každého obchodu podle své důvěry v signál (autonomní pozicování). Pevný kód (Risk Manager) funguje pouze jako "záchranná brzda" – zabraňuje překročení denní ztráty, zakazuje deriváty a vynucuje povinný Stop-Loss u každého nákupu.
...
- **Model Context Protocol (MCP)**: Systém využívá oficiální Python SDK `mcp` pro navázání spojení se subprocess serverem `alpaca-mcp-server`. To umožňuje umělé inteligenci bezpečně a nativně číst data o portfoliu, korporátních událostech, úrokových mírách a definicích aktiv.
- **Asynchronní I/O Architektura**: Celý systém, včetně desítek monitorů na pozadí, je napsán plně asynchronně pomocí `asyncio` a `aiohttp`. Vlákna nejsou blokována čekáním na síťovou odezvu.
...
- **Zpracování Přirozeného Jazyka (NLP)**: Využití knihovny `youtube-transcript-api` k dolování textu z videí a asynchronní parsování webu a sociálních sítí (včetně Wikipedia API a proxy pro Twitter/X sentiment).
